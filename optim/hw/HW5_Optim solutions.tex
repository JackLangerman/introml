...
% \begin{align*}
% 	\nabla J(\wbf) &= \nabla \sum_{i=1}^n g_i(z_i), &z_i &= \xbf^{(i)} \wbf \\
%     &= \sum_{i=1}^n \nabla g_i(z_i), &z_i &= \xbf^{(i)} \wbf \\
%     & \quad \nabla g_i(z_i), &z_i &= \xbf^{(i)} \wbf \\
%     &= g_i'(z_i) z_i', &z_i' &= \xbf^{(i)} \\
%     g_i'(z_i) &= 2 \left(y_i - \frac{1}{z_i} \right) \frac{1}{z_i^2}
% \end{align*}
...
% def loss(A, P, y):
% 	z = np.matmul(A, np.matmul(P, A.T))
%     J = np.sum(z/y - np.log(z))
    
%     dJdz = 1/np.matmul(A,A.T
\begin{python}
def loss(A, P, y):
	n = A.shape[0]
	z = np.zeros(n)
